...
#define ESC_CLRRGB(r, g, b) (struct esc_clr){.tag = ESC_CLRTAG_RGB,  .val.rgb  = {r, g, b}}
#define ESC_CLRID(c)        (struct esc_clr){.tag = ESC_CLRTAG_ID,   .val.id   = c        }
#define ESC_CLRCODE(c)      (struct esc_clr){.tag = ESC_CLRTAG_CODE, .val.code = c + 30   }
...
#define ESC_STRBUF_RING_STACK(p_buf)  \
&(struct esc_strbuf_impl) {           \
	.buf_tag = ESC_STRBUF_RING_STACK, \
	.circular_stack = {               \
		.buf = p_buf,                 \
		.size = sizeof(p_buf)         \
	},                                \
}
#define ESC_STRBUF_RING_HEAP(p_size) ESC_STRBUF_GROWABLE(p_size, 0.0f)
#define ESC_STRBUF_GROWABLE(p_size, p_growth_rate) \
&(struct esc_strbuf_impl) {                        \
...
#ifdef ESC_SHORTHAND
#define MAX_UTF8_CU   ESC_MAX_UTF8_CU
#define MAX_CP_WIDTH  ESC_MAX_CP_WIDTH
#endif
