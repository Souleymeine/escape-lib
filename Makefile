...
TEMPS_FLAGS   := $(if $(filter true yes 1,$(TEMPS)),-save-temps=obj -masm=intel)
...
# For the meaning of the sr, sd, dr, dd suffixes, see test targets bellow
...
.PHONY: cleanlib cleantest mostlyclean distclean clean clean-temps clean-sr clean-sd clean-dr clean-dd
...
$(build_dir)/static/release/%.o: $(src_dir)/%.c | $(build_dir)/static/release
	$(CC) $(TEMPS_FLAGS) $(OBJ_FLAGS) $(BASE_FLAGS) $(RELEASE_FLAGS) $(LTO_FLAG) $< -o $@

$(build_dir)/static/debug/%.o: $(src_dir)/%.c | $(build_dir)/static/debug
	$(CC) $(TEMPS_FLAGS) $(OBJ_FLAGS) $(BASE_FLAGS) $(DEBUG_FLAGS) $< -o $@

$(build_dir)/dynamic/release/%.o: $(src_dir)/%.c | $(build_dir)/dynamic/release
	$(CC) $(TEMPS_FLAGS) $(OBJ_FLAGS) $(BASE_FLAGS) $(RELEASE_FLAGS) -fPIC $< -o $@

$(build_dir)/dynamic/debug/%.o: $(src_dir)/%.c | $(build_dir)/dynamic/debug
	$(CC) $(TEMPS_FLAGS) $(OBJ_FLAGS) $(BASE_FLAGS) $(DEBUG_FLAGS) -fPIC $< -o $@

# ~~~ Specific object targets based on inlcude directives (generated by $(CC) where the corresponding objects live) ~~~
